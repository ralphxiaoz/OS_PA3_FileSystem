...
public class MyFileSystem implements FileSystem {
    ...
    private DirectBlock getDirectBlock(int fd, MODE mode) {
        ...
        // maximum blockNum (starting from 0) supported by each level of indirection
        int directMax = 9;
        int singleMax = directMax + IndirectBlock.COUNT;
        int doubleMax = (int) (singleMax + Math.pow(IndirectBlock.COUNT, 2));
        int tripleMax = (int) (doubleMax + Math.pow(IndirectBlock.COUNT, 3));
        
        if (blockNum > tripleMax) {
        	System.err.println(" Files larger than " + tripleMax + " blocks not supported.");
            System.exit(1);
		}
        
    	// Direct
        if (blockNum <= directMax) {
        ...
        // Single Indirect
        // has 1 Lv1 indirect block.
        if (blockNum > directMax && blockNum <= singleMax) {
        	IndirectBlock singleIndirectBlock = new IndirectBlock();
        	int singleOff = blockNum - 10;

        	// if Lv1 block doesn't exist, create one and write to disk.
        	if (inode.ptr[10] == 0) {
        		inode.ptr[10] = freeMap.find();
        		disk.write(inode.ptr[10], singleIndirectBlock);
			}
        	// read the Lv1 block
        	disk.read(inode.ptr[10], singleIndirectBlock);

        	boolean fresh = singleIndirectBlock.ptr[singleOff] == 0;

            	if(fresh)
                    if(mode == MODE.r)
                        return DirectBlock.hole;
                    else if((singleIndirectBlock.ptr[blockNum - 10] = freeMap.find()) == 0)
                        return null;
            	disk.write(inode.ptr[10], singleIndirectBlock); //write the modified indirect block into disk
                return new DirectBlock(disk, singleIndirectBlock.ptr[singleOff], blockOff, fresh);
		}// end of Single Indirect
  
        // Double Indirect
        // has 1 Lv1 indirect block, 128 Lv2 indirect blocks, 128^2 direct blocks
        if(blockNum > singleMax && blockNum <= doubleMax) {
        	IndirectBlock doubleIndirectBlock_lv1 = new IndirectBlock();
            IndirectBlock doubleIndirectBlock_lv2 = new IndirectBlock();
            int doubleLv2Num = (blockNum - singleMax -1)/IndirectBlock.COUNT; // which Lv2 block
            int doubleLv2Off = (blockNum - singleMax -1)%IndirectBlock.COUNT; // offset in Lv2 block
            
            // if Lv1 block doesn't exist, create one and write to disk.
            if (inode.ptr[11] == 0) {
				inode.ptr[11] = freeMap.find();
				disk.write(inode.ptr[11], doubleIndirectBlock_lv1);
			}
            // read Lv1 block
            disk.read(inode.ptr[11], doubleIndirectBlock_lv1);
            
            // if Lv2 block doesn't exist, create one and write to disk.
            if (doubleIndirectBlock_lv1.ptr[doubleLv2Num] == 0) {
            	doubleIndirectBlock_lv1.ptr[doubleLv2Num] = freeMap.find();
            	disk.write(doubleIndirectBlock_lv1.ptr[doubleLv2Num], doubleIndirectBlock_lv2);
            	disk.write(inode.ptr[11], doubleIndirectBlock_lv1); 
			}
            // read Lv2 block
            disk.read(doubleIndirectBlock_lv1.ptr[doubleLv2Num], doubleIndirectBlock_lv2);
            
            boolean fresh = doubleIndirectBlock_lv2.ptr[doubleLv2Off] == 0;
            
            if(fresh)
                if(mode == MODE.r)
                    return DirectBlock.hole;
                else if((doubleIndirectBlock_lv2.ptr[doubleLv2Off] = freeMap.find()) == 0)
                    return null;
        	disk.write(doubleIndirectBlock_lv1.ptr[doubleLv2Num], doubleIndirectBlock_lv2); 
            return new DirectBlock(disk, doubleIndirectBlock_lv2.ptr[doubleLv2Off], blockOff, fresh);
        } // end of Double Indirect
        
        // Triple Indirect
        // has 1 Lv1 indirect block, 128 Lv2 indirect blocks, 128^2 Lv3 indirect blocks, 128^3 direct blocks.
        if (blockNum > doubleMax && blockNum <= tripleMax) {
        	IndirectBlock tripleIndirectBlock_lv1 = new IndirectBlock();
            IndirectBlock tripleIndirectBlock_lv2 = new IndirectBlock();
            IndirectBlock tripleIndirectBlock_lv3 = new IndirectBlock();
            int tripleLv2Num = (int) ((blockNum - doubleMax -1)/Math.pow(IndirectBlock.COUNT, 2)); // which Lv2 block
            int tripleLv3Num = (int) (((blockNum - doubleMax -1)/IndirectBlock.COUNT)%IndirectBlock.COUNT); // which Lv3 block
            int tripleLv3Off = (int) ((blockNum - doubleMax -1)%IndirectBlock.COUNT); // offset in Lv3 block
            
            // if Lv1 block doesn't exist, create one and write to disk.
            if (inode.ptr[12] == 0) {
				inode.ptr[12] = freeMap.find();
				disk.write(inode.ptr[12], tripleIndirectBlock_lv1);
			}
            // read Lv1 block
            disk.read(inode.ptr[12], tripleIndirectBlock_lv1);
            
            // if Lv2 block doesn't exist, create one and write to disk.
            if (tripleIndirectBlock_lv1.ptr[tripleLv2Num] == 0) {
            	tripleIndirectBlock_lv1.ptr[tripleLv2Num] = freeMap.find();
            	disk.write(tripleIndirectBlock_lv1.ptr[tripleLv2Num], tripleIndirectBlock_lv2);
            	disk.write(inode.ptr[12], tripleIndirectBlock_lv1);
			}
            // read Lv2 block
            disk.read(tripleIndirectBlock_lv1.ptr[tripleLv2Num], tripleIndirectBlock_lv2);
            
            // if Lv3 block doesn't exist, create one and write to disk.
            if (tripleIndirectBlock_lv2.ptr[tripleLv3Num] == 0) {
				tripleIndirectBlock_lv2.ptr[tripleLv3Num] = freeMap.find();
				disk.write(tripleIndirectBlock_lv2.ptr[tripleLv3Num], tripleIndirectBlock_lv3);
				disk.write(tripleIndirectBlock_lv1.ptr[tripleLv2Num], tripleIndirectBlock_lv2);
				disk.write(inode.ptr[12], tripleIndirectBlock_lv1);
			}
            // read Lv3 block
            disk.read(tripleIndirectBlock_lv2.ptr[tripleLv3Num], tripleIndirectBlock_lv3);
            
            boolean fresh = tripleIndirectBlock_lv3.ptr[tripleLv3Off] == 0;
            
            if(fresh)
                if(mode == MODE.r)
                    return DirectBlock.hole;
                else if((tripleIndirectBlock_lv3.ptr[tripleLv3Off] = freeMap.find()) == 0)
                    return null;
        	disk.write(tripleIndirectBlock_lv2.ptr[tripleLv3Num], tripleIndirectBlock_lv3); //write the modified indirect block into disk
            return new DirectBlock(disk, tripleIndirectBlock_lv3.ptr[tripleLv3Off], blockOff, fresh);
        	
		}// end of Triple indirect
